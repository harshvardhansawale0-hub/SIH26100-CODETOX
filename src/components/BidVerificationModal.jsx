...
export default function BidVerificationModal({
  ...
  // Form State
  const [vendorName, setVendorName] = useState('Apex Supplies Ltd.');
  const [category, setCategory] = useState(selectedTender?.category || 'IT Hardware');
  ...
  const [bidAmount, setBidAmount] = useState('₹1.38 Cr');
  const [gstin, setGstin] = useState('27AABCB1234F1Z5');
  const [pan, setPan] = useState('AABCB1234F');
  const [miiDeclared, setMiiDeclared] = useState('68%');
  const [turnoverClaim, setTurnoverClaim] = useState('₹12.4 Cr');
  const [experienceClaim, setExperienceClaim] = useState('5 Years');
  const [msmeRegNo, setMsmeRegNo] = useState('UDYAM-MH-03-0019284');

  // Documents uploaded
  const [uploadedFiles, setUploadedFiles] = useState([
    { name: 'Income_Tax_PAN_Card.pdf', size: '1.2 MB', status: 'Ready' },
    { name: 'GST_Registration_Certificate.pdf', size: '2.4 MB', status: 'Ready' },
  ...
  const verificationStages = [
    { num: 1, title: 'Uploaded Documents OCR & Ingestion', desc: 'Running EasyOCR / Tesseract on PAN, GST, CA Statement & MII' },
    { num: 2, title: 'Entity & Data Extraction', desc: 'Extracting legal names, GSTIN, PAN, turnover numbers & UDIN' },
    { num: 3, title: 'Document Validation & Forensics', desc: 'OpenCV gradient consistency, font analysis & tampering detection' },
    { num: 4, title: 'Cross-Document Matching', desc: 'Checking entity name & PAN consistency across all 5 uploaded documents' },
  ...
  useEffect(() => {
    if (selectedTender) {
      ...
      setCategory(selectedTender.category || 'IT Hardware');
  ...
  const handlePreload = (scenario) => {
    ...
    if (scenario === 'perfect') {
      setVendorName('Apex Supplies Ltd.');
      setGstin('27AABCB1234F1Z5');
      setPan('AABCB1234F');
      setMiiDeclared('68%');
      ...
      setVendorName('Balaji Enterprises');
      setGstin('27BLEPA4411D1Z8');
      setPan('BLEPA4411D');
      setMiiDeclared('41%'); // Class-II
      setTurnoverClaim('₹1.4 Cr'); // Marginal turnover
      setExperienceClaim('2 Years');
      setMsmeRegNo('UDYAM-MH-03-0099812');
      setBidAmount('₹1.42 Cr');
      setUploadedFiles([
        { name: 'Income_Tax_PAN_Card.pdf', size: '1.1 MB', status: 'Ready' },
        { name: 'GST_Registration_Certificate.pdf', size: '2.1 MB', status: 'Ready' },
        { name: 'UDYAM_MSME_Certificate.pdf', size: '780 KB', status: 'Ready' },
        { name: 'CA_Audited_Turnover_FY25.pdf', size: '2.9 MB', status: 'Marginal' },
        { name: 'Make_In_India_Declaration.pdf', size: '590 KB', status: 'Class-II' }
      ]);
    } else if (scenario === 'fraud') {
      setVendorName('UniVend Solutions');
      setGstin('06UNIV0000Z1Z0'); // Suspended
      setPan('ABCDE1234F'); // Mismatch
      setMiiDeclared('12%'); // Under 20%
      setTurnoverClaim('₹80 Lakhs'); // Under 2 Cr
      setExperienceClaim('0.5 Years');
      setMsmeRegNo('UDYAM-HR-00-INVALID');
      setBidAmount('₹1.20 Cr');
      setUploadedFiles([
        { name: 'Income_Tax_PAN_Card.pdf', size: '1.0 MB', status: 'Mismatch Alert' },
        { name: 'GST_Registration_Certificate_Suspended.pdf', size: '2.3 MB', status: 'Suspended' },
        { name: 'UDYAM_MSME_Invalid.pdf', size: '600 KB', status: 'Invalid' },
        { name: 'CA_Turnover_Tampered.pdf', size: '3.0 MB', status: 'Tampering Alert' },
        { name: 'Make_In_India_NonCompliant.pdf', size: '510 KB', status: 'Non-Compliant' }
      ]);
    }
  };
      ...
      >
        ...
        <div className="modal-body" style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem', padding: '1rem 0' }}>
          ...
          <div style={{ background: '#f8fafc', padding: '0.85rem 1rem', borderRadius: '8px', border: '1px solid #e2e8f0' }}>
            <span style={{ fontSize: '0.8rem', fontWeight: '800', color: '#475569', display: 'block', marginBottom: '0.5rem', textTransform: 'uppercase', letterSpacing: '0.5px' }}>
              ⚡ Quick Test Scenarios (Instant Vendor & Document Preloads):
            </span>
            <div style={{ display: 'flex', gap: '0.65rem', flexWrap: 'wrap' }}>
              ...
              >
                🟢 Scenario A: Fully Compliant Bidder (Score 96)
              </button>
              <button
                type="button"
              ...
              >
                🟡 Scenario B: Marginal Flagged Bidder (Score 61)
              </button>
              <button
                type="button"
              ...
              >
                🔴 Scenario C: Non-Compliant / Defaulter (Score 22)
              </button>
            </div>
          </div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      style={{ width: '100%', padding: '0.45rem', borderRadius: '6px', border: '1px solid #cbd5e1', fontSize: '0.85rem', fontWeight: '600' }}
                    />
                  </div>
